...
class newReadingForm(forms.ModelForm): 
	...
	course = forms.ModelChoiceField(queryset=Course.objects.all(), help_text="What course is this for?", required=True)
...
class newSessionForm(forms.ModelForm): 
	...
	reading = forms.ModelChoiceField(queryset=Item.objects.all(), to_field_name="title", help_text="What reading did you work on?", required=True)
	class Meta:
		model = StudySessions
